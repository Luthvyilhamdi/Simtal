@extends('layouts.app')
@section('title', 'Import Baseline dari Excel')
...
<div class="page-header">
    <div class="page-title">Import Baseline dari Excel</div>
    <div class="page-sub">Untuk versi pertama (baseline) — transkrip bagan lama ke template Excel, sistem akan memvalidasi &amp; menampilkan preview sebelum disimpan.</div>
</div>

<div class="mode-banner">
    📥 Ini khusus untuk versi <strong>pertama</strong> di sistem. Setelah versi ini tersimpan (sebagai draft), fitur import baseline tidak akan muncul lagi — versi lanjutan diinput lewat form "Versi Baru" seperti biasa.
</div>
...
<form method="POST" action="{{ route('organisasi.struktur.import.upload') }}" enctype="multipart/form-data">
    @csrf
    ...
    <div class="form-card">
        <div class="section-header">
            ...
            <div class="section-sub">Kolom wajib: kode_sementara, nama_unit, level, parent_kode_sementara, formasi</div>
        </div>
        <div class="file-drop">
            <input type="file" name="file" accept=".xlsx,.xls,.csv" required>
            <div class="template-cols">kode_sementara | nama_unit | level | parent_kode_sementara | formasi</div>
            <div class="form-hint" style="margin-top:10px;">
                Level harus salah satu dari: {{ implode(', ', \App\Http\Controllers\StrukturOrganisasiVersiController::LEVELS) }}.<br>
                parent_kode_sementara kosong = unit paling atas (root).
                Kolom referensi lain di file (grand total, status, keterangan) opsional — boleh ada tapi diabaikan importer.
                File dengan lebih dari 1 sheet juga didukung — sheet yang kolomnya cocok akan otomatis dipakai.
            </div>
        </div>
