...
<style>
    ...
    .stat-badge { display:inline-flex;align-items:center;padding:4px 12px;border-radius:20px;font-size:13px;font-weight:700;background:#dbeafe;color:#1d4ed8; }
    ...
    .edu-card { background:white;border-radius:var(--radius);border:1px solid var(--card-border);box-shadow:var(--card-shadow);padding:16px 20px;display:flex;align-items:center;gap:16px;transition:box-shadow 0.15s; }
    .edu-card:hover { box-shadow:0 4px 16px rgba(0,0,0,0.06); }
    .edu-badge { flex-shrink:0;min-width:64px;text-align:center;padding:8px 12px;border-radius:10px;font-size:14px;font-weight:800;background:#dbeafe;color:#1d4ed8; }
    .edu-body { flex:1;min-width:0; }
    .edu-jurusan { font-size:15px;font-weight:700;color:#111827; }
    .edu-jurusan .muted { color:#9ca3af;font-weight:500; }
    .edu-institusi { font-size:12.5px;color:#6b7280;margin-top:3px;display:flex;align-items:center;gap:6px; }
    .edu-institusi svg { width:13px;height:13px;stroke:#9ca3af;fill:none;stroke-width:2;flex-shrink:0; }
    .edu-actions { display:flex;gap:8px;flex-shrink:0; }

    .icon-btn { width:32px;height:32px;border-radius:8px;border:1px solid #e5e7eb;background:#fff;display:inline-flex;align-items:center;justify-content:center;cursor:pointer;transition:all .12s;flex-shrink:0; }
    ...
    .modal-box { background:white;border-radius:16px;padding:28px;width:100%;max-width:440px;margin:16px;box-shadow:0 20px 60px rgba(0,0,0,0.2);animation:modalIn 0.25s cubic-bezier(0.4,0,0.2,1); }
    ...
    .form-field input, .form-field select { width:100%;border:1px solid #d1d5db;border-radius:9px;padding:10px 12px;font-size:13px;font-family:inherit;background:#fff; }
    .form-field input:focus, .form-field select:focus { outline:none;border-color:#15803d;box-shadow:0 0 0 3px rgba(21,128,61,0.1); }
...
<div class="profile-card">
    ...
    <div class="profile-stats">
        <div class="stat">
            <div class="stat-num green">{{ $riwayat->count() }}</div>
            <div class="stat-label">Jenjang</div>
        </div>
        <div class="stat">
            <div class="stat-badge">{{ $karyawan->jenjang_pendidikan ?: '-' }}</div>
            <div class="stat-label" style="margin-top:6px;">Pendidikan Terakhir</div>
        </div>
    </div>
</div>
...
<div class="page-header">
    <div>
        ...
        <div class="page-sub">Kelola jenjang pendidikan {{ $karyawan->nama }} (SD → S3). "Pendidikan Terakhir" otomatis dari jenjang tertinggi.</div>
    </div>
...
<div class="edu-list">
    ...
    <div class="edu-card">
        <div class="edu-badge">{{ $rp->jenjang }}</div>
        <div class="edu-body">
            <div class="edu-jurusan">{{ $rp->jurusan ?: '' }}@if(!$rp->jurusan)<span class="muted">Tanpa jurusan</span>@endif</div>
            @if($rp->institusi)
            <div class="edu-institusi">
        ...
        <div class="edu-actions">
            <button type="button" class="icon-btn edit" title="Ubah"
                data-id="{{ $rp->id }}" data-jenjang="{{ $rp->jenjang }}"
                data-jurusan="{{ $rp->jurusan }}" data-institusi="{{ $rp->institusi }}"
            ...
            <button type="button" class="icon-btn del" title="Hapus"
                data-url="{{ route('riwayat_pendidikan.destroy', [$karyawan, $rp]) }}"
                data-label="{{ $rp->jenjang }}{{ $rp->jurusan ? ' - '.$rp->jurusan : '' }}"
...
<div class="empty-state">
    <svg viewBox="0 0 24 24"><path d="M22 10v6M2 10l10-5 10 5-10 5z"/><path d="M6 12v5c3 3 9 3 12 0v-5"/></svg>
    <p>Belum ada history pendidikan</p>
    <span style="font-size:12px;color:#9ca3af;">Klik "Tambah Pendidikan" untuk menambahkan</span>
</div>
@endif
...
<div class="modal-backdrop" id="modalForm">
    <div class="modal-box">
        <div class="modal-title" id="formTitle">Tambah Pendidikan</div>
        <form id="eduForm" method="POST">
            @csrf
            <input type="hidden" name="_method" id="eduMethod" value="POST">
            <div class="form-field">
                <label>Jenjang <span style="color:#dc2626;">*</span></label>
                <select name="jenjang" id="fJenjang" required>
            ...
            <div class="form-field">
                <label>Jurusan</label>
                <input type="text" name="jurusan" id="fJurusan" placeholder="cth: Teknik Mesin" maxlength="255">
            </div>
            <div class="form-field">
                <label>Institusi / Sekolah</label>
                <input type="text" name="institusi" id="fInstitusi" placeholder="cth: Universitas Gadjah Mada" maxlength="255">
            </div>
            <div class="modal-actions" style="margin-top:20px;">
                <button type="button" class="modal-btn cancel" onclick="closeForm()">Batal</button>
                <button type="submit" class="modal-btn green" id="formSubmit">Simpan</button>
            </div>
        </form>
    </div>
</div>
...
<div class="modal-backdrop" id="modalHapus">
    <div class="modal-box center">
        ...
        <div class="modal-title">Hapus History Pendidikan?</div>
...
@push('scripts')
<script>
const STORE_URL  = "{{ route('riwayat_pendidikan.store', $karyawan) }}";
const UPDATE_TPL = "{{ route('riwayat_pendidikan.update', [$karyawan, '__ID__']) }}";
...
function openForm(mode, data = {}) {
    ...
    if (mode === 'edit') {
        document.getElementById('formTitle').textContent = 'Ubah Pendidikan';
        document.getElementById('eduMethod').value = 'PUT';
        form.action = UPDATE_TPL.replace('__ID__', data.id);
        document.getElementById('fJenjang').value   = data.jenjang || '';
        document.getElementById('fJurusan').value   = data.jurusan || '';
        document.getElementById('fInstitusi').value = data.institusi || '';
    } else {
        document.getElementById('formTitle').textContent = 'Tambah Pendidikan';
        document.getElementById('eduMethod').value = 'POST';
        form.action = STORE_URL;
        form.reset();
    }
    document.getElementById('modalForm').classList.add('show');
    document.body.style.overflow = 'hidden';
    setTimeout(() => document.getElementById('fJenjang').focus(), 50);
}
function closeForm() {
...
function openHapus(url, label) {
    ...
    document.getElementById('hapusDesc').innerHTML =
        'Kamu akan menghapus <strong>' + label + '</strong>.<br>Tindakan ini tidak dapat dibatalkan.';
...
@if($errors->any())
    ...
    @if(old('_method') !== 'PUT')
        document.getElementById('fJenjang').value   = @json(old('jenjang'));
        document.getElementById('fJurusan').value   = @json(old('jurusan'));
        document.getElementById('fInstitusi').value = @json(old('institusi'));
    @endif
@endif
